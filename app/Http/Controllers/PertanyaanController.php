    ...
    if ($request->ajax()) {
        ...
        return Datatables::of($data)
            ...
            ->addColumn('aksi', function($row){
                $btn = '<button class="btn btn-warning btn-sm" onclick="modalEdit(\'/admin/pertanyaan/' . $row->pertanyaan_id . '/edit_ajax\')">Edit</button>';
                $btn .= ' <button class="btn btn-danger btn-sm" onclick="modalDelete(\'/admin/pertanyaan/' . $row->pertanyaan_id . '/delete_ajax\')">Hapus</button>';
                return $btn;
            })
            ->rawColumns(['aksi'])
            ->make(true); // ⬅️ WAJIB agar JSON valid
    }
